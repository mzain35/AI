        ...
        if node not in visited:
            ...
            for n in graph.neighbors(node):
                if n not in visited:
                    ...
                    # print(n, parent[n], bool(parent[n]))
                    if n in parent and parent[n][1] > total_cost:
                        parent[n] = (node, total_cost)
                    elif n not in parent:
                        parent[n] = (node, total_cost)
...
def greedy_best_first_search(graph, start, goal):
    visited = set()
    queue = PriorityQueue()
    queue.put((graph.get_heuristic(start), start))
